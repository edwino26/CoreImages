...
plt.figure(figsize=(15,9))
plt.subplot(191)
plt.plot (df.GR_EDTC,df.DEPTH,'g',cg1,h,'c.',lw=0.5)
...
plt.plot (df.Vclay,df.DEPTH,'m',clays,prof,'ro',lw=0.5)
plt.axis([0,1, top,bottom])
plt.gca().invert_yaxis()
plt.gca().yaxis.set_visible(False)
plt.grid(True)
...
plt.subplot(194)
plt.title('Porosity \n  Core Por.')
plt.plot (df.porosity,df.DEPTH,'m',poro,long,'c*',lw=0.5)
...
plt.plot (df.grain_density,df.DEPTH,'y',norm,long,'g>',lw=0.5)
...
#Basic Archie
plt.subplot(196)
plt.plot (df.Sw_a1,df.DEPTH,'c',sw_core,long,'m.',lw=0.5)
plt.title('$SW_A$')
...
plt.subplot(197)
plt.plot (df.Sw_p1,df.DEPTH,'r',sw_core,long,'m.',lw=0.5)
plt.title('$SW_P$')
...
#Waxman-Smits
plt.subplot(198)
plt.plot (df.SwWS,df.DEPTH,'g',sw_core,long,'m.',lw=0.5)
plt.title('$SW_W$')
...
#Simandoux
plt.subplot(199)
plt.plot (df.Swsim1,df.DEPTH,'y',sw_core,long,'m.',lw=0.5)
plt.title('$SW_S$')
...
plt.show()
# %%
df.insert(22, "WELL", 'T6')
df.head()
with pd.ExcelWriter('T6.xlsx') as writer:  
    df.to_excel(writer, sheet_name='T6_data')
