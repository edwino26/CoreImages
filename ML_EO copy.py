...
scaler = MinMaxScaler()
window = 5
T2.GRAY = T2.GRAY.rolling(window , win_type='gaussian').mean(std=3)
T6.GRAY = T6.GRAY.rolling(window , win_type='gaussian').mean(std=3)
U18.GRAY = U18.GRAY.rolling(window , win_type='gaussian').mean(std=3)
...
#df_val = T2[logset].copy().rename(columns={"GR_EDTC": "GR", "AT90": "logRT", "RHOZ": "RHOB", "WELL": "Well"}).set_index('Well')
...
df_val['logRT'] = df_val['logRT'].apply(lambda x: math.log10(x))
...
del df
df = df_smogn.copy()
df.to_excel('./ML_Results/df_smogn.xlsx','Smogn')
#-------------------------------
...
df_val['Pay'] = df_val['GRAY'].apply(lambda x: 1 if x> 170 else 0)
...
#df = df.append(aa).append(ab).append(ac).append(ad)


data = df.copy()
data= data[["GR", "RHOB",	"logRT",	"DTCO",	"NPHI",	"GRAY",	"Pay","DEPT", "Well"]]
df_val =  df_val[["GR", "RHOB",	"logRT",	"DTCO",	"NPHI",	"GRAY",	"Pay","DEPT", "Well"]]
data.head(100)
...
X_val = df_val.iloc[:, [0,1,2,3,4]]
y_val= df_val.iloc[:, [target_col]]  
...
error = pd.DataFrame(index=['RMSE',  'RMSEval', 'MSE','MSEval'], columns=Methods)
...
df_val[method] = Ypredict 
df_val.head()
mseval = mean_squared_error(y_val, Ypredict)
rmseval = mean_squared_error(y_val, Ypredict, squared=False)
error.loc['MSEval', method] = mseval.round()
error.loc['RMSEval', method] = rmseval.round()
plt.figure(figsize=(4,4))
plt.subplot(1,1,1)
plt.plot(y_val, Ypredict, 'ro')
...
df_val.sort_values('DEPT').to_excel('./ML_Results/T2_validation/Results.xls')
