...
def  MLPRegressorGS(X, y):
    architecture = [x for x in itertools.product((10,50,100,30),repeat=4)]
    #  'activation': 'relu',
    #  'alpha': 0.0001,
    #  'batch_size': 'auto',
    #  'beta_1': 0.9,
    ...
    rgr = MLPRegressor()
    rgr.get_params() 
    tuned_parameters = [{'hidden_layer_sizes':architecture, 'learning_rate':np.linspace(0.01,3, 10), 'alpha':np.linspace(0.01,0.99, 10) }]
    gsr= GridSearchCV(rgr, tuned_parameters, scoring=score_RMSE, refit=True)
    gsr.fit(X, y)
    print(gsr.best_params_)
    return gsr.best_estimator_    
