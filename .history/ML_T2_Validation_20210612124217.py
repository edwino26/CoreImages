...
plt.subplot(1,no_plots,ct, facecolor='#242424')
corte= 170
...
plt.fill_between(PAY_Gray_scale.values.reshape(-1),res.DEPT.values.reshape(-1), color='#c5e1e6', alpha=0.8)
plt.axis([0.01,0.0101,top,bottom])
plt.xticks(fontsize=8)
plt.gca().invert_yaxis()
plt.gca().xaxis.set_visible(False)
...
ct+=1
plt.subplot(1,no_plots,ct, facecolor='#474747')
...
plt.fill_between(PAY_Gray_scale2.values.reshape(-1),res.DEPT.values.reshape(-1), color='#c5e1e6', alpha=0.8)
#plt.fill_between(np.ones(shape=PAY_Gray_scale2.values.reshape(-1).shape),res.DEPT.values.reshape(-1), color='c', alpha=0.8)
