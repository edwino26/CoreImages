...
rgr =  MLPRegressor(n_estimators=100, random_state=0, learning_rate=0.1, max_depth=5,loss='ls', alpha=0.9)
...
rgr =  GradientBoostingRegressor(n_estimators=ne, random_state=0, learning_rate=lr, max_depth=md,loss='ls', alpha=al)
...
axs[0].plot(y, y, 'blue'); axs[0].set_xlabel('True '+option); axs[0].set_ylabel('Predicted '+option)
axs[0].plot(y, y_pred_train, 'ko')
axs[0].text(0.001, 0.08, 'RMSE = '+str(round(rmse_train,2)), verticalalignment='bottom', horizontalalignment='right', transform=ax.transAxes,color='black', fontsize=10)
axs[1].plot(y_test, y_test,  'blue')
axs[1].plot(y_test, y_pred_test, 'go')
axs[1].set_title('Test '+option)
