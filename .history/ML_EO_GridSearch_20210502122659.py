...
def  SVRGS(X, y):
    ...
    rgr = SVR()
    rgr.get_params() 
    tuned_parameters = [{'C':np.linspace(0.01,0.99, 10), 'epsilon':np.linspace(0.01,0.3, 4), 'kernel':'linear'},
                       {'C':np.linspace(0.01,0.99, 10), 'epsilon':np.linspace(0.01,0.3, 4), 'kernel':'sigmoid'}]
    gsr= GridSearchCV(rgr, tuned_parameters, scoring=score_RMSE, refit=True)
    gsr.fit(X, y)
    print(gsr.best_params_)
    return gsr.best_estimator_
