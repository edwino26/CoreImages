...
with open('./ML_Results/'+pkl_filename, 'wb') as file:
    pickle.dump(rgr, file)

#Load Model    
with open('./ML_Results/'+pkl_filename, 'rb') as file:
    model = pickle.load(file)
score = model.score(x_val, y_val)
print("Test score: {0:.2f} %".format(100 * score))
Ypredict = model.predict(scaler.transform(x_val))

Ypredict = pd.DataFrame(Ypredict, columns=['Gray_pred'], index=x_val.index)
