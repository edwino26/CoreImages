...
#plt.fill_between(T2_x.PAY_poupon.T,T2_x.DEPTH.T, color='r', alpha=0.8)



plt.fill_between(T2_x.PAY_poupon.values.reshape(-1),T2_x.DEPTH.values.reshape(-1), color='r', alpha=0.8)
plt.axis([0.01,0.011,top,bottom])
plt.xticks(fontsize=8)
plt.gca().invert_yaxis()
...
ct+=1
PAY_Gray_scale = res['GRAY']
PAY_Gray_scale.mask(PAY_Gray_scale>= corte, 1 )
PAY_Gray_scale.mask(PAY_Gray_scale< corte, 0 )
...
plt.fill_between(PAY_Gray_scale.values.reshape(-1),res.DEPT.values.reshape(-1), color='c', alpha=0.8)
plt.axis([0,0.001,top,bottom])
plt.xticks(fontsize=8)
plt.gca().invert_yaxis()
plt.gca().xaxis.set_visible(False)
...
ct+=1
#PAY_Gray_scale2 = res['RandomForest'].apply(lambda x: 1 if x<corte else 0)
PAY_Gray_scale2 = res['RandomForest']
PAY_Gray_scale2.mask(PAY_Gray_scale>= corte, 1 )
PAY_Gray_scale2.mask(PAY_Gray_scale< corte, 0 )

h_ML = integrate(PAY_Gray_scale2.values, 0.5)
plt.subplot(1,no_plots,ct)
plt.plot (res.DEPT,'c',lw=0.5)
plt.title('$Validations$',fontsize=8)
plt.fill_between(PAY_Gray_scale2.values.reshape(-1),res.DEPT.values.reshape(-1), color='c', alpha=0.8)
plt.axis([0,0.001,top,bottom])
plt.xticks(fontsize=8)
plt.gca().invert_yaxis()
plt.gca().xaxis.set_visible(False)
...
# %%
pay = pd.DataFrame(columns=['Poupon', 'Waxman_Smits', 'Simandooux', 'Machine_L', 'True_UV'])
