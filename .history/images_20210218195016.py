...
for k in range(len(uvFiles)): #Loop through various files containing images
    ...
    for i in range(cores_per_image):
        ...
        if i == 0 and k == 0:
            ImgStack = pd.Dataframe(data=vc)
        else:
            ImgStack = ImgStack.append(pd.Dataframe(data=vc))
...
plt.axis([90, 200, doo, dnn]);
plt.gca().invert_yaxis();
#plt.gca().invert_xaxis()

plt.subplot2grid((1, 10), (0, 3), colspan=7)
plt.imshow(vc_gray[:,80:120], aspect='auto', origin='upper', extent=[0,1,dnn,doo], cmap='gray');
plt.axis([0, 1, doo, dnn]);
plt.gca().invert_yaxis()
