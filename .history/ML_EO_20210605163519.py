...
#%% Merge using Index and Plot
y_pred_train = pd.DataFrame(y_pred_train, columns=['y_pred_train'])
#NEED TO RECOVER PRED INDEXES HERE!!!!

full = train.merge(y, left_index=True, right_index=True)
full = full.merge(y_pred_train, left_index=True, right_index=True)
print(full)


fig, axs = plt.subplots(1, 1, constrained_layout=True)
axs.set_title('Train '+option)
#axs.plot(full['GRAY_y'], full['y_pred_train'], 'blue'); axs.set_xlabel('True '+option); axs.set_ylabel('Predicted '+option)
axs.plot(full['GRAY_y'], full['y_pred_train'], 'ko')
# axs[1].plot(y_test, y_test,  'blue')
# axs[1].plot(y_test, y_pred_test, 'go')
# axs[1].set_title('Test '+option)
# axs[1].text(1, 0.03, 'MSE = '+str(round(mse,2)), verticalalignment='bottom', horizontalalignment='right', transform=axs[1].transAxes,color='green', fontsize=10)
