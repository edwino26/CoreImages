...
Well = 'U18'
cores_per_image = 6
uvFiles = glob.glob('./Photos/U18/*.tif')
filedos = []
...
for k in range(len(uvFiles)): #Loop through various files containing images
    ...
    for i in range(cores_per_image):
        ...
        if i == 0:# and k == 0:
            ...
            vc = cv2.vconcat([vc, crop_img,ac])
            
        vc_gray = cv2.cvtColor(vc, cv2.COLOR_BGR2GRAY)   
        crop_name = str(int(fname[0:3]) + (core_length * i)) + ".tif"
    ...
    concat_name = fname[0:3] + "-" + fname[4:8] + ".tif"
