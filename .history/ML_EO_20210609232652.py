...
if res == 0.5:
    ...
    print("Resolution not supported. Enter 0.25 or 0.5")


def round_depth(number):
    return round(number * numb, 0) / numb  #Use 2 to round to closest 0.5, 4 to 0.25
...
if balance == 1:
    data_smogn = smogn.smoter( 
        ...
        pert = 0.01,
        k = 5,
        rel_thres = 0.1,          ## relevance threshold considered rare (pos real)
        rel_method = "auto",      ## relevance method ("auto" or "manual")
        rel_xtrm_type = "both",   ## distribution focus ("high", "low", "both")
        rel_coef = 1.0,           ## coefficient for box plot (pos real)
        rel_ctrl_pts_rg = None
    )

    ## Check changes in target variable distribution
    smogn.box_plot_stats(data['GRAY'])['stats']
    smogn.box_plot_stats(data_smogn['GRAY'])['stats']
    ## plot y distribution 
    sns.kdeplot(data['GRAY'], label = "Original")
    sns.kdeplot(data_smogn['GRAY'], label = "Modified")
    print("Size of unbalanced DF ===> ", data.shape)
    print("Size of balanced DF ===> ", data_smogn.shape)
    del data
    data = data_smogn.copy()


train, test = train_test_split(data_smogn, test_size=0.3, random_state=42)
...
if GRIDSEARCH == 'on':
    ...
    rgr = RandomForestRegressor(n_estimators=1000, criterion='mse')
    rgr.fit(X, np.ravel(y))
