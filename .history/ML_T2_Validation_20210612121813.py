...
plt.plot (T2_x.GR_EDTC,T2_x.DEPTH,'g', lw=2)
plt.fill_between(T2_x.DEPTH.values.reshape(-1),T2_x.GR_EDTC.values.reshape(-1), 0*T2_x.DEPTH.values.reshape(-1),color='g', alpha=0.8)
...
plt.title('$TEST Data$',fontsize=8)
plt.fill_between(PAY_Gray_scale2.values.reshape(-1),res.DEPT.values.reshape(-1), color='k', alpha=0.8)
plt.axis([0.01,0.0101,top,bottom])
plt.xticks(fontsize=8)
plt.gca().invert_yaxis()
plt.gca().xaxis.set_visible(False)
