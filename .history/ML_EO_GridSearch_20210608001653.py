...
def  RandomForestRegressorGS(X, y):
    ...
    rgr.get_params() 
    tuned_parameters = [{'n_estimators':np.array([10000, 50000, 100000])}]
...
def  GradientBoostingRegressorGS(X, y):
    ...
    rgr.get_params() 
    tuned_parameters = [{'alpha':np.linspace(0.01,0.99, 10), 'learning_rate':np.linspace(0.01,0.1, 3), 'n_estimators':np.array([10, 50, 100, 500, 1000])}]
    gsr= GridSearchCV(rgr, tuned_parameters, scoring=score_RMSE, refit=True)
    gsr.fit(X, y)
    print(gsr.best_params_)
    return gsr.best_estimator_    
...
def  MLPRegressorGS(X, y):
    #architecture = [x for x in itertools.product((10,50,100),repeat=3)]
    architecture = [(10, 50, 100), (100, 10, 50), (50, 100, 10)]
    ...
    rgr = MLPRegressor()
    rgr.get_params() 
    tuned_parameters = [{'hidden_layer_sizes':architecture, 'learning_rate_init':[0.001, 0.01, 0.1], 'alpha':np.linspace(0.01,0.99, 4) }]
    gsr= GridSearchCV(rgr, tuned_parameters, scoring=score_RMSE, refit=True)
    gsr.fit(X, y)
    print(gsr.best_params_)
    return gsr.best_estimator_    
