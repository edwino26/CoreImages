...
Well = 'U18'
cores_per_image = 6
uvFiles = glob.glob('./Photos/U18/*.jpg')
print(uvFiles)
filedos = []

for l in range(len(uvFiles)):
    
    filedos.append(int(uvFiles[l][13:16]))
    print(filedos)
...
for k in range(len(uvFiles)): #Loop through various files containing images
    ...
    fname = uvFiles[k][13:25]
    # Picture path
    img = cv2.imread(uvFiles[k])
    do = int(float(fname[0:5]))
    dn = int(float(fname[7:11]))  
    ...
    #Convert apparent depths to real depths
    # Core Depth conversions necessary:
    # 804-822 => 803.85 - 818.9
    # 822-840 => 864.75 - 879.75
    # 840-858 => 925.8  - 940.75

    di1= np.arange(803.85,818.9,0.25)
    di2= np.arange(864.75,879.75,0.25)
    di3= np.arange(925.8,940.75,0.25)
...
sub.to_excel("./Photos/U18/JPG/Processed_Images_U18.xlsx",sheet_name=Well) 
...
#Upon revision, best way to plot core photos is to create a large array 
# containing all possible depths and then plugging in intervals with image data
