...
#
variables= ['DEPT', 'GR', 'RHOB', 'RT', 'DTCO', 'NPHI', 'GRAY']
fig, axes = scatterplotmatrix(df[df['Well']=='T2'].values, figsize=(10, 8), alpha=0.5)
fig, axes = scatterplotmatrix(df[df['Well']=='T6'].values, fig_axes=(fig, axes), alpha=0.5)
fig, axes = scatterplotmatrix(df[df['Well']=='U18'].values, fig_axes=(fig, axes), alpha=0.5, names=variables)
plt.tight_layout()
plt.show()
